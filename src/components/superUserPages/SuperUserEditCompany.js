...
export default function SuperUserEditCompany() {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(
        `http://54.93.232.137:8080/getcompanybyname?name=${searchName}`,
        {
          method: "GET",
          headers: {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const response = await fetch(
        `http://54.93.232.137:8080/superuser/updatecompany?id=${company.id}`,
        {
          method: "PUT",
          headers: {
  ...
  const handleDelete = async () => {
    try {
      const token = sessionStorage.getItem("token");
      const response = await fetch(
        `http://54.93.232.137:8080/superuser/deletecompany?id=${company.id}`,
        {
          method: "DELETE",
          headers: {
