...
export default function Appointment() {
    ...
    const fetchProviderInfo = async (id) => {
      ...
      try {
        const response = await fetch(
          `http://3.71.9.9:8080/getproviders?companyId=${id}`,
          {
            method: "GET",
            headers: { "Content-Type": "application/json" },
          }
  ...
  React.useEffect(() => {
    if (selectedCompany.id) {
      const fetchProviderInfo = async () => {
        ...
        try {
          const response = await fetch(
            `http://3.71.9.9:8080/getproviders?companyId=${selectedCompany.id}`,
            {
              method: "GET",
              headers: { "Content-Type": "application/json" },
            }
    ...
    if (selectedProvider.email && selectedDate) {
      const fetchAppointments = async () => {
        ...
        try {
          ...
          const response = await fetch(
            `http://3.71.9.9:8080/getproviderappall?providerEmail=${selectedProvider.email}&date=${formattedDate}`,
            {
              method: "GET",
              headers: { "Content-Type": "application/json" },
            }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(
        `http://3.71.9.9:8080/activateapp?appointmentID=${selectedAppointment.id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
