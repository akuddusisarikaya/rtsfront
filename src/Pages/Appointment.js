...
export default function Appointment() {
  ...
  React.useEffect(() => {
    const fetchCompanies = async () => {
      ...
      setError(null);
      try {
        const response = await fetch("http://3.123.49.33:8080/getcompanies", {
          method: "GET",
  ...
  React.useEffect(() => {
    if (selectedCompany.id) {
      const fetchProviderInfo = async () => {
        ...
        try {
          const response = await fetch(
            `http://3.123.49.33:8080/getproviders?companyId=${selectedCompany.id}`,
            {
              method: "GET",
              headers: { "Content-Type": "application/json" },
            }
    ...
    if (selectedProvider.email && selectedDate) {
      const fetchAppointments = async () => {
        ...
        try {
          ...
          const response = await fetch(
            `http://3.123.49.33:8080/getproviderappall?providerEmail=${selectedProvider.email}&date=${formattedDate}`,
            {
              method: "GET",
              headers: { "Content-Type": "application/json" },
            }
