...
export default function UserProfileEdit() {
  ...
  React.useEffect(() => {
    const fetchUserProfile = async () => {
      ...
      try {
        ...
        const response = await fetch(`http://54.93.232.137:8080/protected/userprofile?email=${userEmail}`, {
          method: 'GET',
          headers: {
  ...
  const handleSave = async () => {
    try {
      const token = sessionStorage.getItem('token');
      const response = await fetch("http://54.93.232.137:8080/protected/user", {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
